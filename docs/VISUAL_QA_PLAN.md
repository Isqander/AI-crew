...
- [ ] `sandbox/Dockerfile.browser`: образ с Playwright + Chromium + Node.js
- [ ] `sandbox/models.py`: поля browser, collect_screenshots, app_start_command, app_ready_timeout
- [ ] `sandbox/executor.py`: выбор образа (browser vs standard), сбор скриншотов
- [ ] `sandbox/server.py`: новые поля в response (screenshots, browser_console, network_errors)
- [ ] Docker-compose: build sandbox-browser образа
- [ ] Unit-тесты: модели, выбор образа, сбор скриншотов

#### Browser Test Runner
- [ ] `graphs/dev_team/tools/browser_runner.py`: шаблон runner-скрипта
- [ ] Runner: установка зависимостей, запуск приложения, ожидание готовности
- [ ] Runner: запуск Playwright тестов, сбор скриншотов
- [ ] Runner: structured JSON output
- [ ] Unit-тесты: парсинг, генерация runner-скрипта

#### QA Agent: test_ui()
- [ ] `agents/qa.py`: `has_ui()` — определение UI-проекта по tech_stack / code_files
- [ ] `agents/qa.py`: `test_ui()` — генерация + выполнение + анализ
- [ ] `agents/qa.py`: `_generate_browser_test()` — LLM генерирует Playwright тест
- [ ] `agents/qa.py`: `_analyse_browser_results()` — LLM анализирует скриншоты + результаты
- [ ] `agents/qa.py`: `merge_results()` — объединение code + browser results
- [ ] `agents/qa.py`: `_extract_code_block()` — извлечение кода из LLM-ответа
- [ ] `prompts/qa.yaml`: `generate_browser_test` template
- [ ] `prompts/qa.yaml`: `analyse_browser_results` template
- [ ] `state.py`: `browser_test_results: NotRequired[dict]`
- [ ] `graph.py`: qa_agent node вызывает test_ui() при наличии UI
- [ ] `USE_BROWSER_TESTING` env var (default: true)
