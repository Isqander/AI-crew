...
| **PostgreSQL** | Database | 5432 | 5432 | Checkpoints, users, metadata |
...
| 5432 | PostgreSQL | Открыт | Для прямого доступа (позже закроем) |
...
POSTGRES_DB=aicrew
POSTGRES_PORT=5432                   # Порт наружу (позже закроем)
...
|---|------|-------------|---------|
| 1 | PostgreSQL :5432 | Не ограничиваем по IP сейчас, закроем порт позже | Принято. Порт открыт, в проде — закроем firewall |
...
services:
  ...
  aegra:
    ...
    environment:
      AUTH_TYPE: noop
      DATABASE_URL: postgresql+asyncpg://${POSTGRES_USER:-aicrew}:${POSTGRES_PASSWORD}@postgres:5432/${POSTGRES_DB:-aicrew}
      LANGFUSE_ENABLED: "true"
  ...
  gateway:
    ...
    environment:
      ...
      DATABASE_URL: postgresql://${POSTGRES_USER:-aicrew}:${POSTGRES_PASSWORD}@postgres:5432/${POSTGRES_DB:-aicrew}
