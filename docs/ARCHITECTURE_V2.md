...
> 
> Дата: 16 февраля 2026 (обновлено: принцип Scripts-over-manual, рефакторинг)
...
- **Scripts-over-manual** *(предложение, ожидает одобрения)* — всё, что можно автоматизировать скриптами, должно делаться программно: деплой, миграции, проверки здоровья, сборка конфигов. Это снижает вероятность ошибок и уменьшает потребление токенов при работе агентов
...
```
    ...
    │  ├── app1.deploy.mysite.ru   (custom domain)    │
    │  ├── app2.deploy.mysite.ru   (custom domain)    │
    │  ├── OR app1.31.59.58.143.nip.io  (fallback)   │
    │  └── ...                                         │
    │                                                  │
    │  Domain strategy:                                │
    │    DEPLOY_DOMAIN set → *.deploy.mysite.ru        │
    │    DEPLOY_DOMAIN empty → *.IP.nip.io (default)   │
    │                                                  │
    │  Repo strategy: single-repo (DEPLOY_SINGLE_REPO)│
    │    Each project → branch project/{app-name}      │
    │    Secrets set once on repo level                 │
    │                                                  │
    │  Prefect Server :4200 (мониторинг, опционально) │
    └─────────────────────────────────────────────────┘
```
...
| 7 | **Домены деплоя** | **Гибрид: nip.io + свой домен** | `DEPLOY_DOMAIN` env: пусто = nip.io (zero-config), задан = свой субдомен (профессиональный). Wildcard DNS `*.deploy.mysite.ru` настраивается один раз |
| 8 | **Стратегия repos** | **Один repo (single-repo)** | Один `DEPLOY_SINGLE_REPO`, каждый проект — ветка `project/{app-name}`. Секреты на repo один раз. Фаза 2: миграция на динамическое создание repos |
| 9 | **Секреты для deploy** | **Platform-level injection** | `DEPLOY_VPS_*` лежат в env AI-crew. Платформенный код записывает их в GitHub Secrets через API (PyNaCl шифрование). ИИ-агенты никогда не видят значения секретов |
