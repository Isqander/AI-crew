...
### Принципы:
1. **Интеграционные тесты > Unit тесты** для часто меняющихся компонентов
2. **Smoke tests** для проверки базовой работоспособности
3. **Глубокие unit тесты** только для стабильных компонентов

### Почему не все покрыто unit тестами?
Агенты (промпты, LLM модели, парсинг) меняются очень часто. Глубокие unit тесты
с моками будут ломаться при каждом изменении, замедляя итерации. Вместо этого:
- ✅ Smoke tests проверяют, что агенты создаются
- ✅ Интеграционные тесты проверяют реальный флоу
- ✅ Быстрая итерация без постоянной правки тестов
...
### test_agents.py
Smoke tests для агентов (намеренно упрощены):
- **Инициализация**: Проверка, что все агенты могут быть созданы
- **Промпты**: Проверка существования всех prompt файлов

**Примечание:** Глубокие unit тесты внутренней логики агентов намеренно не включены,
так как агенты часто изменяются (промпты, модели, логика). Для проверки реального
поведения используйте интеграционные тесты в `test_integration.py`.
...
### Статусы линтинга в графе `dev_team`

- `clean` — линт чистый, флоу продолжается
- `warnings` — только некритичные предупреждения (non-blocking), флоу продолжается
- `issues` — блокирующие ошибки линтера, возврат в `Developer`
- `error` / `skipped` — инфраструктурная ошибка/пропуск, флоу продолжается

По умолчанию некритичными считаются коды `E501,W291,W293,I001`.
Их можно переопределить через env `LINT_NON_BLOCKING_CODES`.
